...
class ProductionManagementAgent {
  ...
  async generateCaptions(productionData) {
    ...
    if (isShort) {
      ...
      const audioDuration = productionData.estimatedDuration || 30;

      // Extract the EXACT narration text sent to TTS as the single source of truth
      let spokenNarration = '';
      if (productionData.assets?.script?.ttsPath) {
        try {
          spokenNarration = await fs.readFile(productionData.assets.script.ttsPath, 'utf8');
        } catch (_err) {
          spokenNarration = '';
        }
      }
      if (!spokenNarration) {
        spokenNarration = ShortsKaraokeCaptions.extractTextFromScript(productionData.script);
      }
      ...
      const karaokeResult = await ShortsKaraokeCaptions.processCaptions({
        ...
        baseName: `${productionData.id}_captions`,
        options: {
          aspectRatio: '9:16',
          isShort: true,
          providerTimings: this.aiVideoGenerator?.lastNarrationResult?.wordTimings || this.aiVideoGenerator?.lastWordTimings
        }
      });

      productionData.assets.captions = {
        path: karaokeResult.srtPath,
        assPath: karaokeResult.assPath,
        format: 'ass+srt',
        language: 'en',
        autoGenerated: true,
        karaoke: true,
        wordCount: karaokeResult.wordCount,
        phraseCount: karaokeResult.phraseCount,
        verified: karaokeResult.verification?.valid ?? true
      };

      productionData.timeline.captionsGenerated = new Date().toISOString();
      return karaokeResult.assPath;
    }

    const captionsPath = path.join(captionsDir, `${productionData.id}_captions.srt`);
  ...
  async assembleVideo(productionData) {
    ...
    try {
      ...
      if (productionData.assets?.script?.ttsPath) {
        try {
          narrationText = await fs.readFile(productionData.assets.script.ttsPath, 'utf8');
        } catch (_err) {
          narrationText = '';
        }
      }
      if (!narrationText && productionData.script) {
        const { ShortsKaraokeCaptions } = require('../utils/shorts-karaoke-captions');
        narrationText = ShortsKaraokeCaptions.extractTextFromScript(productionData.script);
      }
      ...
      productionData.assets.finalVideo = {
        ...
        aspectRatio: isShort ? '9:16' : '16:9',
        format: 'mp4',
        hook: isShort ? this.aiVideoGenerator.lastHookResult : null,
